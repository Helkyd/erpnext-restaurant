class OrderItem {
  ...
  //02-06-2025; With DeepSeek help
  updateDishSideLabels() {
    if (!this.form_editor || !this.data.dish_sides) return;
    
    const sides = this.data.dish_sides;
    
    // Update labels for all dish sides
    for (let i = 0; i < 6; i++) {
        const fieldName = `dish_side_${String(i+1).padStart(2, '0')}`;
        const label = sides[i]?.item_name || "";
        
        if (this.form_editor.fields_dict[fieldName]) {
            // Update both the properties and the field directly
            this.form_editor.field_properties[fieldName].label = label;
            this.form_editor.fields_dict[fieldName].df.label = label;
            this.form_editor.fields_dict[fieldName]._label = label;
            
            // Force a refresh of the field
            this.form_editor.fields_dict[fieldName].refresh();

            console.log(`Setting ${fieldName} label to:`, label, 
              'Current value:', this.form_editor.fields_dict[fieldName]?.get_value());            
        }
    }
  }   
