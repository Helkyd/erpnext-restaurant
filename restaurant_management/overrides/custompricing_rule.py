...
def get_pricing_rule_for_item(args, doc=None, for_validate=False):
	...
	print ('ARGS ', args)
	print ('validate ', for_validate)
	print ('prcingRULES ', args.get('pricing_rules'))
	...
	if pricing_rules:
		#HELKYD 
		print ('TESTS Weekdasy')
		#Monday is 0, Tuesday 1, Wednesday 2, etc
		print ('TESTAAAAA ')
		print (len(pricing_rules))
		print (isinstance(pricing_rules, str))
		if not 'day_monday' in pricing_rules[0]:
			pricing_rule = frappe.get_cached_doc("Pricing Rule", pricing_rules[0])
			print (pricing_rule.day_monday)
			print ('00000000TIME from ', pricing_rule.from_time)
			print ('TO TIME ', pricing_rule.to_time)
			print ('NOW TIME ', frappe.utils.nowtime())
			from datetime import datetime
			print (datetime.strptime(str(frappe.utils.nowtime()),'%H:%M:%S.%f').strftime("%H:%M:%S"))
			print ('h ', datetime.strptime(str(frappe.utils.nowtime()),'%H:%M:%S.%f').strftime("%H:%M:%S") >= datetime.strptime(str(pricing_rule.from_time),'%H:%M:%S').strftime("%H:%M:%S"))
			print ('b ', str(frappe.utils.nowtime()) <= str(pricing_rule.to_time))
			can_run_today = False
			#FIX 11-06-2025
			if (pricing_rule.day_monday == 0 and pricing_rule.day_tuesday == 0 and pricing_rule.day_wednesday == 0 and 
				pricing_rule.day_thursday == 0 and pricing_rule.day_friday == 0 and pricing_rule.day_saturday == 0 and pricing_rule.day_sunday == 0):
					can_run_today = True
					print ('Verifica se pode correr!!!!!!!')
					print ('Pode correr nao tem RULES per DAY!!!!')

			elif datetime.strptime(str(frappe.utils.nowtime()),'%H:%M:%S.%f').strftime("%H:%M:%S") >= datetime.strptime(str(pricing_rule.from_time),'%H:%M:%S').strftime("%H:%M:%S") and datetime.strptime(str(frappe.utils.nowtime()),'%H:%M:%S.%f').strftime("%H:%M:%S") <= datetime.strptime(str(pricing_rule.to_time),'%H:%M:%S').strftime("%H:%M:%S"):
				if pricing_rule.day_monday == 1 and frappe.utils.now_datetime().weekday() == 0:
					#Check time
					can_run_today = True
				elif pricing_rule.day_tuesday == 1 and frappe.utils.now_datetime().weekday() == 1:
					#Check time
					can_run_today = True
				elif pricing_rule.day_wednesday == 1 and frappe.utils.now_datetime().weekday() == 2:
					#Check time
					can_run_today = True
				elif pricing_rule.day_thursday == 1 and frappe.utils.now_datetime().weekday() == 3:
					#Check time
					can_run_today = True
				elif pricing_rule.day_friday == 1 and frappe.utils.now_datetime().weekday() == 4:
					#Check time
					can_run_today = True
				elif pricing_rule.day_saturday == 1 and frappe.utils.now_datetime().weekday() == 5:
					#Check time
					can_run_today = True
				elif pricing_rule.day_sunday == 1 and frappe.utils.now_datetime().weekday() == 6:
					#Check time
					can_run_today = True		
			...
			print (pricing_rules[0].day_monday)
			print ('TIME from ', pricing_rules[0].from_time)
			...
			if (pricing_rules[0].from_time and pricing_rules[0].to_time) and (datetime.strptime(str(frappe.utils.nowtime()),'%H:%M:%S.%f').strftime("%H:%M:%S") >= datetime.strptime(str(pricing_rules[0].from_time),'%H:%M:%S.%f').strftime("%H:%M:%S") and datetime.strptime(str(frappe.utils.nowtime()),'%H:%M:%S.%f').strftime("%H:%M:%S") <= datetime.strptime(str(pricing_rules[0].to_time),'%H:%M:%S.%f').strftime("%H:%M:%S")):
				if pricing_rules[0].day_monday == 1 and frappe.utils.now_datetime().weekday() == 0:
					#Check time
					can_run_today = True
				elif pricing_rules[0].day_tuesday == 1 and frappe.utils.now_datetime().weekday() == 1:
					#Check time
					can_run_today = True
				elif pricing_rules[0].day_wednesday == 1 and frappe.utils.now_datetime().weekday() == 2:
					#Check time
					can_run_today = True
				elif pricing_rules[0].day_thursday == 1 and frappe.utils.now_datetime().weekday() == 3:
					#Check time
					can_run_today = True
				elif pricing_rules[0].day_friday == 1 and frappe.utils.now_datetime().weekday() == 4:
					#Check time
					can_run_today = True
				elif pricing_rules[0].day_saturday == 1 and frappe.utils.now_datetime().weekday() == 5:
					#Check time
					can_run_today = True
				elif pricing_rules[0].day_sunday == 1 and frappe.utils.now_datetime().weekday() == 6:
					#Check time
					can_run_today = True
		...
		#HELKYD 10-06-2025; Check if can run today
		#FIX 11-06-2025; Check if all are 0
		if 'day_monday' in pricing_rules[0]:
			if (pricing_rules[0].day_monday == 0 and pricing_rules[0].day_tuesday == 0 and pricing_rules[0].day_wednesday == 0 and 
			pricing_rules[0].day_thursday == 0 and pricing_rules[0].day_friday == 0 and pricing_rules[0].day_saturday == 0 and pricing_rules[0].day_sunday == 0):
				can_run_today = True
				print ('Verifica se pode correr!!!!!!!')
				print ('Pode correr nao tem RULES per DAY!!!!')
